...
class CandidateControllerTest {
    ...
    @Test
    public void whenRequestCandidateByIdAndGetPageWithCandidateOne() {
        var candidateExpected = new Candidate(1, "test1", "desc1", now(), 1, 2);
        when(candidateService.findById(1)).thenReturn(Optional.of(candidateExpected));
    ...
    @Test
    public void whenPostCandidateUpdateAndRedirectToCandidatesPage() throws IOException {
        var candidateExpected = new Candidate(1, "test1", "desc1", now(), 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var candidateArgumentCaptor = ArgumentCaptor.forClass(Candidate.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
    ...
    @Test
    public void whenPostCandidateUpdateAndGetErrorMessage() throws IOException {
        var expectedMessage = "Кандидат с указанным идентификатором не найден";
        var candidateExpected = new Candidate(1, "test1", "desc1", now(), 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var candidateArgumentCaptor = ArgumentCaptor.forClass(Candidate.class);
    ...
    @Test
    public void whenRequestCandidateDeleteByIdAndRedirectToCandidatesPage() {
